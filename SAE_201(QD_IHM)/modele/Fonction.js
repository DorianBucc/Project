...
function estEspace(lettre) {
    return lettre === " ";
}
export function estPositif(nombre) {
    return nombre > 0;
}
export function immat_valide(immat) {
    if (immat.length !== 7) {
        throw new ErrorData("Immat invalide");
    }
    if (estLettre(immat[0]) &&
        estLettre(immat[1]) &&
        estChiffre(immat[2]) &&
        estChiffre(immat[3]) &&
        estChiffre(immat[4]) &&
...
export function date_valide(date_immat) {
...
export function marque_valide(marque) {
    if (marque.length < 3) {
...
export function modele_valide(modele) {
    if (modele.length < 1) {
...
export function permis_valide(permis) {
...
export function nature_valide(nature) {
...
//   }
// }
export function nom_valide(nom) {
    if (nom.length < 2) {
...
export function prenom_valide(prenom) {
    if (prenom.length < 2) {
