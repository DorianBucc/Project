...
type TConducteur = { [key: string]: UnConducteur };
...
class LesConducteurs {
  ...
  //   );
  // }
  private load(result: APIsql.TdataSet): TConducteur {
    // à partir d’un TdataSet, conversion en tableau d’objets UnConducteur
    let conducteurs: TConducteur = {};
    for (let i = 0; i < result.length; i++) {
      const item: APIsql.TtabAsso = result[i];
      const conducteur = new UnConducteur(
        item["no_permis"],
        item["date_permis"],
        item["nom"],
        item["prenom"]
  ...
  //   ]);
  // }
  update(conducteur: UnConducteur): boolean {
